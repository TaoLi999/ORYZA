...
!-----Formal parameters
      INTEGER       ITASK , IUNITD, IUNITL, CROPSTA, IDOY, I, K,IYEAR,EMD,EMYR
      LOGICAL       OUTPUT, TERMNL
      CHARACTER (*) FILEI1, FILEIT, FILEI2
      CHARACTER (*) ESTAB
      REAL          DOY , YEAR,TIME, DELT  , LAT  , RDD, TRC, RBH,RT, DIF,RBW,RSWP,RSWA, TempR, KDF2
      REAL          TMMN, TMMX, TMAXC, TMINC, TKLT  , ZRTMS, LRSTRS, LDSTRS, LESTRS, NRT, SSTRS  
                    !SSTRS is the salt stress index varying from 0 to 1 for servere to none stress
      REAL          PCEW, CPEW, DAE , LAIROL, ZRT  , DVS, NSLLV, RNSTRS, WCL(10), WL0,DAYSFL,DAYSPI,DAYSFL1,DAYSFL2
      
      !DAYSFL, DAYSPI are the days to flowering and panicle initiation FOR PHENOLOGY STUDY
!-----Local variables
      LOGICAL DLEAF, DROUT, INQOBS  , GRAINS

      INTEGER IMX, IMXX 
      PARAMETER (IMX=40, IMXX = 800)
      INTEGER ILDRLV, ILEFFT, ILFLVT, ILFSHT, ILFSOT, ILFSTT,MOPPFL
      !MOPPFL IS USED FOR GIVEN DAYLENGTH IN PHOTOPERIOD SENSITIVE STAGE
      INTEGER ILKDFT, ILKNFT, ILSLAT,ILNMIN,ILNMAX 
      INTEGER ILREDF, ILSSGA, ILCO2A
      REAL    DRLVT(IMX) , EFFTB(IMX), SLATB(IMX), MOPPF(200) !MOPPF is fixed DAYL in photoperoid
      ...
      REAL    SHADET,SHADDELY  !Index of crop tolerate shade, 0.5 is the default, 0 to 1 increasing tolerance, and the days for shaded leaf to servive
      INTEGER   SWIRTR 
      REAL     FSWTD,  LLRT, ULRT,SWIRTRF,ULLS,NDSENS
      !COLD CAUSED STERILITY AND EARLY STOP GROWTH, added by TAOLI 28Jan 2014
      REAL    COLDREP,COLDTT,CONSEC,BOOTCOLD,BCDAYS,COLDMIN,COLDEAD
      INTEGER COLDSTOP, OLDCOLD !The accumulative cold days for stop growth after DVS=1
!-----TAOLI, JUNE 10, 2009
      REAL    DROUGHT1, DROUGHT2  !Total drought stress index from day at main field to flowering, and from flowering to maturity

      REAL    SHOUR,  EHOUR,  TTEMPD, TTEMPN, TCHANG,SDAY,TSYEAR,EDAY,TEYEAR, BETA
	  REAL    XTAV,   XTMIN,   XTMAX,   XHU, XTAVD !OUTPUT, THE NET CHANGE
	  INTEGER ISTEMC, CONTRM, sl
	  logical RDINQR, TEMPC, INITIALF,RDINAR				
!-----TAOLI, 6 AUG, 2010
...
!        Read management parameters
         ...
         END IF
         !Read climate change parameters
         IF(RDINQR('TMPSB')) THEN
		 ...
		 ELSE
		    ILCO2A = 0
         END IF
!--------If use external input daylength in a given period, TAOLI 17 MAY 2014
         IF(RDINQR('MOPPF')) THEN
             CALL RDAREA('MOPPF ',MOPPF,200,MOPPFL)
         ELSE
             MOPPFL = 0
         END IF
...
!        Read model parameters
		 ...
		 IF(RDINQR('CTSTER')) THEN
		    CALL RDSREA('CTSTER', CTSTER)			!special root length cm/g C
		 ELSE
		    CTSTER = 36.6
		 END IF
...
!        TEST SPATIAL ADJUSTMENT ON PHENOLOGY, TAOLI, 9FEB 2014
        IF(PV%ISPHENAD) THEN
            TBD = TBD-PV%PLAALA
            TBLV = TBLV -PV%PLAALA
            TMD = TMD-PV%PLAALA
...
!--------ADD BY TAOLI, JUNE 17
		...
		IF(RDINQR('SHADET')) THEN
            ...
            SHADDELY = 10.0 +5.0* (SHADET/0.5-1.0)
		ELSE
			SHADET=0.5;SHADDELY = 10.0
		ENDIF
		SHADET = max(0.01,min(0.99,SHADET)) !limited SHADET between 0.01 and 0.99
		!TAOLI, 31 oct 2012
...
!        Read tables
         ...
         IF(ILFLVT.NE.ILFSOT) THEN
             CALL FATALERR ('Crop data file','FLVTB and FSOTB do not have same data pairs.')
         ELSE
             ILFSTT = ILFLVT
             DO I = 1,INT(ILFLVT/2.0)
                 
                 IF(FLVTB((I-1)*2+1).NE.FSOTB((I-1)*2+1)) THEN
                     CALL FATALERR ('Crop data file','FLVTB and FSOTB do not set in same growth stage.')
                 ELSEIF((FLVTB((I-1)*2+2).GT.1.0).OR.(FLVTB((I-1)*2+2).LT.0.0) &
                        .OR.(FSOTB((I-1)*2+2).GT.1.0).OR.(FSOTB((I-1)*2+2).LT.0.0)) THEN
                     CALL FATALERR ('Crop data file','FLVTB and FSOTB has >1.0 or <0.0 partitioning number.')
                 ELSEIF((FLVTB((I-1)*2+2)+FSOTB((I-1)*2+2)).GT.1.0) THEN
                     Y3 = 1.0/(FLVTB((I-1)*2+2)+FSOTB((I-1)*2+2))
                     FLVTB((I-1)*2+2) = FLVTB((I-1)*2+2) * Y3
                     FSOTB((I-1)*2+2) = FSOTB((I-1)*2+2) * Y3
                     FSTTB((I-1)*2+1) = FLVTB((I-1)*2+1)
                     !CALL FATALERR ('Crop data file','FLVTB + FSOTB > 1.0.')       
                 ELSE
                     FSTTB((I-1)*2+1) = FLVTB((I-1)*2+1)
                     FSTTB((I-1)*2+2) = 1.0 - FLVTB((I-1)*2+2) - FSOTB((I-1)*2+2) 
                 END IF
             END DO
         END IF
...
!--------Added by TAOLI, June 17, 2009
	    IF(PV%PROOT_NUTRIENT) THEN
             ...
             ELSEIF(RDINQR('TKLPI')) THEN
                 CALL RDSREA('TKLPI',TKL(1))
                 TKL(1) = TKL(1)/1000.0
                 SL = 1
             END IF
		     SL = PV%PNL
		     IF(RDINQR('SANDX')) THEN
                 ...
                 ELSE
                     CALL RDSREA('SANDX',PV%PSAND(1))		!soil sand content
			         CALL RDSREA('CLAYX',PV%PCLAY(1))		!soil clay content
                     CALL RDSREA('BD',PV%PBD(1))
                 END IF
             ...
             IF(RDINQR('DPLOWPAN')) THEN
		        CALL RDSREA('DPLOWPAN', PV%PPLOWDEPTH) 
                IF(PV%PPLOWDEPTH.LE.0.0) THEN
                    PV%PPLOWDEPTH=SUM(TKL(1:SL))
                END IF
             ELSE
                IF(RDINQR('NLPUD')) THEN
                    CALL RDSINT('NLPUD', K) 
		            PV%PPLOWDEPTH = SUM(TKL(1:K))
                ELSE
                    PV%PPLOWDEPTH= SUM(TKL(1:SL))
				END IF
	         ENDIF	         
		...
		!---- INITIAL ROOT VARIABLES
			...
			NROOTC=0.0; NROOTN=0.0;PV%TOOWET=0;PV%TOODRY=0;PV%GERMRATE=0.0;PV%GERMDAYS=0.0
...
!--------Initialize state variables
         ...
         CO2ADAY = 0.0
         DAYSFL = -1.0;DAYSPI=-1.0;DAYSFL1=-1.0;DAYSFL2=-1.0
         SUMROOTN = 0.0
...
!		TAOLI, JULY 23, 2013
        IF(INITIALF) THEN
            ...
            ELSE
                CALL RDSREA('ROOTCI',ROOTC(1))
	            CALL RDSREA('ROOTNI',ROOTN(1))
            END IF
            ...
            END IF      
        END IF
!-------If all initialization is success
        WRITE (*,*) 'INITIAL SUCCESS'
!===================================================================*
!     RATE CALCULATION SECTION                                      *
!===================================================================*
      ELSE IF (ITASK.EQ.2) THEN
      !Climate change on air temperature section moved from ORYZA1 into MODEL, 21Sept 2013
      !----------Computation of weather variables BECAUSE OF SEED BED COVER
        IF ((CROPSTA .LE. 2).AND.(CROPSTA .GE. 1)) THEN
            TMPCOV = TMPSB; TMAX = TMMX+TMPCOV; TMIN = TMMN+TMPCOV
        ELSE
            TMPCOV = 0.;TMAX = TMMX;TMIN = TMMN
        END IF	       
	!--------Re-initialize weights and LAI at day of emergence		   	    
         IF (CROPSTA .EQ. 1) THEN
            ...
            IF (ESTAB.EQ.'DIRECT-SEED') THEN
                IF((PV%TOOWET.GE.5).OR.(PV%TOODRY.GE.5)) THEN
                    PV%GERMRATE=0.0
                END IF
                LAI= LAPE * NPLDS*max(0.0,PV%GERMRATE)/PV%GERMDAYS  !LAST TWO TERMS FOR GERMINATION RATE IN DIRECT-SEED
                XXX = 0.0	
                DO I=1, SL
                    XXX = XXX + PV%PDLAYER(I)   !In mm
			        IF(XXX.GE.(ZRTI*1000.0)) THEN  !ZRTI is in mm
                        ROOTC(I) = ROOTC(I) + WRTI 
                        ROOTN(I) = ROOTN(I) + 0.02*WRTI 
                        EXIT
                    END IF
                END DO 
            END IF
...
!=======SKIP ALL RATE CALCULATIONS BEFORE EMERGENCE
        IF (CROPSTA .GE. 1) THEN
           PV%TOTALGROWTHDAYS =PV%TOTALGROWTHDAYS+1 
...
!          vegetative growth phase (=> extra root growth)
           ...
           IF((TEMPC).AND.((YEAR*1000+DOY).GE.(TSYEAR*1000+SDAY)).AND.&
                ((YEAR*1000+DOY).LE.(TEYEAR*1000+EDAY))) THEN
				 CALL TSHIFT(TMAX,TMIN,DAYL, SHOUR, EHOUR, TTEMPD, TTEMPN,TCHANG, &
...
!----------The net change has been calculated here, so recalculate TAV and TAVD
		    TAV = TAV + XTAV; TAVD = TAVD + XTAVD;   HU=HU+XHU
           DTR  = RDD
!----------Counter for days after emergence
           RDAE = 1.
		    
           CALL SUBCD2 (COLDMIN,CROPSTA,TAV,TIME,NCOLD)
!----------Modified for external feeding in DAYL, 17 MAY 2014
            IF(MOPPFL>0) THEN
                IF((DOY.GE.MOPPF(1)).AND.(DOY.LE.MOPPF(MOPPFL-1))) THEN
                    DAYL =MAX(0.0, LINT2('MOPPF',MOPPF,MOPPFL,DOY)-0.9)
                END IF
            END IF
...
!----------Effect of drought stress on development rate
           IF (DVS.LT.1.0) THEN
			  DVEW =MIN(1.0, HU/(HU+1.5*FSWTD*ULLS*(1.0-LESTRS**FSWTD)))  !RENEWED by TAOLI, 11Apri 2014    
!              DVEW = LESTRS + (DVS*(1.-LESTRS))  Bas Bouman's function
              DVEW = 1.  !above function was disabled by setting DVEW=1.
           ELSE IF (DVS.GE.1.) THEN
			  DVEW=MAX(1.0,1.5*FSWTD *(1.0-LESTRS**FSWTD))  !RENEWED by TAOLI, 11Apri 2014 
              DVEW = 1.
           END IF
           DVR = DVR*DVEW
...
!BB: Changed according to SUCROS2
       ...
       IF (GRAINS) THEN
  		   ...
  		   IF(GGR.GE.(PWRR-WRR))THEN
	  		    FSO = MIN(1.0,MAX(0.,(PWRR-WRR)/NOTNUL((GCR*FSH))))
			    FST = 1.-FSO-FLV
		   ENDIF
	   END IF
!           TAOLI, 22Feb 2012
...
!----------USE NIGHT.F90 TO CALCULATE XHU, THEN RECALCULATE HULV
			 IF((TEMPC).AND.((YEAR*1000+DOY).GE.(TSYEAR*1000+SDAY)).AND.&
                ((YEAR*1000+DOY).LE.(TEYEAR*1000+EDAY))) THEN
				 CALL TSHIFT(TMAX,TMIN,DAYL, SHOUR, EHOUR, TTEMPD, TTEMPN,TCHANG, &
					ISTEMC, CONTRM, TBLV,30.0,42.0,HULV, XTAV, XTMIN, XTMAX, XHU,XTAVD)
			 ELSE
...
!            CALL OUTDAT (2,0,'WRTDC',SUM(RRDCL))
!            CALL OUTDAT (2,0,'WRTDN',SUM(RRDNL))
...
!-----------Leaf area index and total area index (leaves + stems)
            ...
            IF(SHADET.LT.1.0) THEN
                ...
                LAIRD = LOG(LAIRD*DAYLP*3600.0/RDD)/KDF    !LOG(LAIRD*DAYLP*3600.0/RDD/KDF)
                LAIRD = MAX(0.0, LAI- MAX(0.0, -LAIRD))  
            ELSE
                LAIRD = 0.0
            ENDIF
            LAI = MAX(0.0,LAI-LAIRD/SHADDELY)
            LAIRD = (LAIRD/SLA/SHADDELY)/DELT                          		
            ALAI   = LAI+0.5*SAI
...
!----------TAOLI, APRIL 2, 2009               
                ...
                ROOTMAXN = 0.5*LINT2('NMAXLT',NMAXLT,ILNMAX,DVS)
...
!--------CHECK IF THE WEIGHT OF ROOT FROM WHOLE AND LAYERS ARE THE SAME
                IF((SUM(ROOTC).GT.0.0).AND.(SUM(ROOTN).GT.0.0)) THEN
...
!-----------Root length
             ...
             ENDIF
            IF(DAYSFL1.LT.1) THEN    !TAOLI, 28 JULY 2013. For the key stage of starting flowering DVS = 0.8
			    IF(DVS.GE.0.8) THEN
			        DAYSFL1 = DAE
			    END IF
            ENDIF
            IF(DAYSFL.LT.1) THEN    !TAOLI, 28 JULY 2013. For flowering peak
			    IF(DVS.GE.1.0) THEN
			        DAYSFL = DAE
			    END IF
            ENDIF
			IF(DAYSFL2.LT.1) THEN    !TAOLI, 28 JULY 2013. For flowering ending
			    IF(DVS.GE.1.2) THEN
			        DAYSFL2 = DAE
			    END IF
            ENDIF
            IF(DAYSPI.LT.1) THEN    !TAOLI, 8 oct 2016
			    IF(DVS.GE.0.65) THEN
			        DAYSPI = DAE
			    END IF
			ENDIF
...
!========END OF SKIP WHOLE RATE CALCULATIONS BEFORE EMERGENCE
         END IF
        !Set total drought index, TAOLI, 31MAR 2015
            IF(CROPSTA.LE.3) THEN
                DROUGHT1=0.0;DROUGHT2=0.0
            ELSE
                IF(DVS.GE.1.0) THEN
                    DROUGHT2 = DROUGHT2 + LRSTRS + LDSTRS + LESTRS + PCEW                    
                ELSE
                    DROUGHT1 = DROUGHT1 + LRSTRS + LDSTRS + LESTRS + PCEW
                END IF
            END IF           
...
!        Terminal output
         ...
         IF(LEN_TRIM(PV%OPSTRING).GT.1) THEN
           ...
           IF(INDEX(PV%OPSTRING,'DAYSPI').GT.0) CALL OPSTOR ('DAYSPI', DAYSPI) 
           IF(INDEX(PV%OPSTRING,'DAYSFL1').GT.0) CALL OPSTOR ('DAYSFL1', DAYSFL1) 
           IF(INDEX(PV%OPSTRING,'DAYSFL').GT.0) CALL OPSTOR ('DAYSFL', DAYSFL) 
           IF(INDEX(PV%OPSTRING,'DAYSFL2').GT.0) CALL OPSTOR ('DAYSFL2', DAYSFL2) 
           ...
           IF(INDEX(PV%OPSTRING,'COLDTT').GT.0) CALL OPSTOR ('COLDTT',COLDTT)
           IF(INDEX(PV%OPSTRING,'CONSEC').GT.0) CALL OPSTOR ('CONSEC',CONSEC)
           IF(INDEX(PV%OPSTRING,'BOOTCOLD').GT.0) CALL OPSTOR ('BOOTCOLD',BOOTCOLD)
           IF(INDEX(PV%OPSTRING,'BCDAYS').GT.0) CALL OPSTOR ('BCDAYS',BCDAYS)
           IF(INDEX(PV%OPSTRING,'NSP').GT.0) CALL OPSTOR ('NSP',NSP) 
         ELSE 
           ...
           CALL OPSTOR ('DAYSPI', DAYSPI)
           CALL OPSTOR ('DAYSFL1', DAYSFL1)
           CALL OPSTOR ('DAYSFL', DAYSFL)
           CALL OPSTOR ('DAYSFL2', DAYSFL2)
