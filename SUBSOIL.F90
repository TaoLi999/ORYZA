...
!--------Save flux (mm/d)
         PERC = 10.*(F1+F2)/2.
	  ELSEIF(SWITKH.EQ.3) THEN			!THIS SECTION WAS ADDED BY TAOLI, 27 MAY, 2010
!-----------SPAW FUNCTION FOR HYDRAULIC CONDUCTIVITY
		 N = SPAWB(NLPUD+1); M=SPAWA(NLPUD+1)
		 F = PV%PSWC(NLPUD+1);FX=PV%PWCST(NLPUD+1);GX=PV%PWCFC(NLPUD+1)
		 HS=0.0
   !-----------Estimated flux through saturated compartment
            F1 = KST(NLPUD)*((HT-HS)/DZL+1.)
 
!-----------Estimated flux through unsaturated compartment
			! 1) estimated the water potential under the WCL
			IF(F.GE.GX) THEN
				A=-(33.0-(F-GX)*33.0/(FX-GX))*10.0
			ELSE
				A=-M*(F)**(-N)*10.0
			ENDIF
			! 2) Estimated the conductivity in unstaturated compartment
			HLP1=KST(NLPUD+1)*(F/FX)**(3+2*N)
			! 3)Estimated the flux through unsaturated compartment
            F2 = HLP1*((HT-A)/TKL(NLPUD+1)+1.0)
			DF=tkl(nlpud)+tkl(nlpud+1)	
			PERC = 10.0*(F1*tkl(nlpud)/DF+F2*tkl(nlpud+1)/DF)
      END IF
...
!-----------Power function conductivity
		 ...
		 ELSEIF(SWITKH.EQ.3) THEN
			IF(MS.LE.1.0) KMS = KST(I)
			IF(MS.GT.1.0) THEN
				CALL SUSPAW1(I,2,WCST,PV%PWCFC(I),PV%PWCWP(I),PV%PWCAD(I),WCL,MS)
				KMS = KST(I)*(WCL/WCST)**(3+2*SPAWB(I))
			ENDIF
...
!----------------------------------------------------------------------*
!  SUBROUTINE SUSPAW1                                                  *
!  Developed by TAOLI, 9 JUNE, 2010                                    *
!  Purpose: SUSPAW1 calculates volumetric soil water content from      *
!           soil water suction, and vice versa BY SAXTON METHOD.       *
!           Various options areoffered.								   *
!                                                                      *
! FORMAL PARAMETERS:  (I=input,O=output,C=control,IN=init,T=time)      *
! name   type meaning (unit)                                     class *
! ----   ---- ---------------                                    ----- *
...
! SWIT4   I4  Switch to set request MS(WCL) or WCL(MS) (-)          I  *
! WCST    R4  Array of water content saturation / layer (cm3 cm-3)  I  *
! WCAD	  R4  Soil water content at air dry			    (cm3/cm3)   I  *
! WCFC    R4  Soil water content at field capacity      (cm3/cm3)   I  *
! WCWP    R4  Soil water content at wilting point       (cm3/cm3)   I  *
! WCL     R4  Array of actual water content / layer (cm3 cm-3)     I/O *
! MS      R4  Soil water suction (cm)                              I/O *
!                                                                      *
!  SUBROUTINES called:  SUERR                                          *
...
!                                                                      *
!***********************************************************************
	SUBROUTINE SUSPAW1(I,SWIT4,WCST,WCFC, WCWP, WCAD1, WCL,MS)
	  USE SPAW
	  USE HYDCON
...
!-----Formal parameters
      INTEGER I   , SWIT4
      REAL    WCST, WCL, MS, WCFC, WCAD1, WCWP
...
!-----Local variables
	  ...
	  IF (SWIT4.EQ.1) THEN
!--------Suction calculated from water content
         IF (WCL.LT.WCAD1.OR.WCL.GT.WCST) &
              CALL SUERR(3,WCL,WCAD1,WCST)
         IF (WCL.GT.WCSTRP(I)) THEN
!           It is assumed that MS remains zero during shrinkage
            MS = 0.
         ELSE
!-----------Saxton option
			IF(WCL.GE.WCFC) THEN            
				MS   = (33.0-(WCL-WCFC)*33.0/(WCST-WCFC))*10.0		!CONVERT FROM KPA INTO CM BY TIME 10.0
			ELSE
				MS = HELP1*(WCL)**(-HELP2)*10.0
			ENDIF
         END IF
      ELSE IF (SWIT4.EQ.2) THEN
!--------Water content calculated from suction
         IF (MS.LT.-TINY.OR.MS.GT.1.E8) CALL SUERR(4,MS,0.,1.E8)
!--------SAXTON option
         MS = MS/10.0			!CONVERT MS FROM CM INTO KPA
		 IF(MS.LE.33.0) THEN
			WCL = WCFC - (33.0-MS)*(WCST-WCFC)/33.0
		 ELSEIF(MS.GE.1000000.0) THEN
			WCL = WCAD1
		 ELSE
			WCL = (MS/HELP1)**(-1/HELP2)
		 ENDIF
