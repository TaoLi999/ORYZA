...
!------For communication with ORYZA subroutine
       ...
       TKLT = TKLPI/1000.
       WCLQT(1) = WCLP
!+      update values of global variables in public_module, TAOLI, 15 Sept, 509
            pv%pnl = nl; pv%pdlayer(1) = tkl(1)*1000.0;PV%PPLOWDEPTH = TKL(1)
            pv%pwcst(1) = wcst(1);      pv%pwcfc(1) = wcfc(1)
            pv%pwcwp(1) = wcwp(1);      pv%pwcad(1) = wcad(1)
!+      end section, TAOLI
!============================================================*
...
!------Reduction of EVSC when no more ponded water
       IF (WL0 .LE. 0) THEN
          ...
          EVSD = MIN(EVSC, 0.6*EVSC*(SQRT(DSLR+1.)-SQRT(DSLR))+RAIN+IR+SSOILIN)
...
!------- Calculate moisture suction in KPa (FROM PADY)
        IF (WCLP.GE.WCFC(1)) THEN
           FACT    = MAX(0.,MIN(1.,(WCST(1)-WCLP)/(WCST(1)-WCFC(1))))
           MSUC(1) = 10.**(FACT*2.0)
           IF (WCLP.GE.WCST(1)) MSUC(1) = 0.
        ELSE IF (WCLP.GE.WCWP(1).AND.WCLP.LT.WCFC(1)) THEN 
           FACT    = MAX(0.,MIN(1.,(WCLP-WCWP(1))/(WCFC(1)-WCWP(1))))
           MSUC(1) = 10.**(4.2-FACT*2.2)
        ELSE IF (WCLP.LT.WCWP(1)) THEN
           FACT    = MAX(0.,MIN(1.,(WCLP-WCAD(1))/(WCWP(1)-WCAD(1))))
...
! Water balance check: inflows-outflows - change in storage should be less than 10 mm.
       CHECK = (RAINCUM1+IRCUM1+SSINCUM1)&
         ...
         END IF
!+            update values of global variables in public_module
            pv%pwl0 = wl0;          pv%prunoff = runof*delt
            pv%pdrain = perc * delt;pv%pevap = evsw * delt
            pv%ptrans = trwp * delt;pv%pwflux(1) = PERC/10.0*DELT !pv%pswc(1)-wclqt(1) !PERC mm/day,pwflux in cm/day
            pv%pswc(1) = wclqt(1)
!+            end section      
...
!         CALL OPSTOR ('PERCCUM1', PERCCUM1)
          IF(LEN_TRIM(PV%OPSTRING).GT.1) THEN 
             IF (ESTAB.EQ.'DIRECT-SEED') THEN
                IF(INDEX(PV%OPSTRING,'RAINCUM').GT.0) CALL OPSTOR ('RAINCUM' , INT(100*RAINCUM2 )/100.0)
                IF(INDEX(PV%OPSTRING,'IRCUM').GT.0) CALL OPSTOR ('IRCUM', INT(100*IRCUM2)/100.0)
                IF(INDEX(PV%OPSTRING,'RUNOFCUM').GT.0) CALL OPSTOR ('RUNOFCUM', INT(100*RUNOFCUM2)/100.0)
                IF(INDEX(PV%OPSTRING,'TRWCUM').GT.0) CALL OPSTOR ('TRWCUM'  , INT(100*TRWCUM2  )/100.0)
                IF(INDEX(PV%OPSTRING,'EVSWCUM').GT.0) CALL OPSTOR ('EVSWCUM' , INT(100*EVSWCUM2 )/100.0)
                IF(INDEX(PV%OPSTRING,'PERCCUM').GT.0) CALL OPSTOR ('PERCCUM', INT(100*PERCCUM2)/100.0)
             END IF
             IF (ESTAB.EQ.'TRANSPLANT') THEN
                IF(INDEX(PV%OPSTRING,'RAINCUM').GT.0) CALL OPSTOR ('RAINCUM' , INT(100*RAINCUM3 )/100.0)
                IF(INDEX(PV%OPSTRING,'IRCUM').GT.0) CALL OPSTOR ('IRCUM', INT(100*IRCUM3)/100.0)
                IF(INDEX(PV%OPSTRING,'RUNOFCUM').GT.0) CALL OPSTOR ('RUNOFCUM', INT(100*RUNOFCUM3)/100.0)
                IF(INDEX(PV%OPSTRING,'TRWCUM').GT.0) CALL OPSTOR ('TRWCUM'  , INT(100*TRWCUM3  )/100.0)
                IF(INDEX(PV%OPSTRING,'EVSWCUM').GT.0) CALL OPSTOR ('EVSWCUM' , INT(100*EVSWCUM3 )/100.0)
                IF(INDEX(PV%OPSTRING,'PERCCUM').GT.0) CALL OPSTOR ('PERCCUM', INT(100*PERCCUM3)/100.0)
    !           CALL OPSTOR ('SSOUTCUM', SSOUTCUM3)
    !           CALL OPSTOR ('SSINCUM', SSINCUM3)
              END IF   
          ELSE
             IF (ESTAB.EQ.'DIRECT-SEED') THEN
                CALL OPSTOR ('RAINCUM' , INT(100*RAINCUM2 )/100.0)
                CALL OPSTOR ('IRCUM', INT(100*IRCUM2)/100.0)
                CALL OPSTOR ('RUNOFCUM', INT(100*RUNOFCUM2)/100.0)
                CALL OPSTOR ('TRWCUM'  , INT(100*TRWCUM2  )/100.0)
                CALL OPSTOR ('EVSWCUM' , INT(100*EVSWCUM2 )/100.0)
                CALL OPSTOR ('PERCCUM', INT(100*PERCCUM2)/100.0)
             END IF
             IF (ESTAB.EQ.'TRANSPLANT') THEN
                CALL OPSTOR ('RAINCUM' , INT(100*RAINCUM3 )/100.0)
                CALL OPSTOR ('IRCUM', INT(100*IRCUM3)/100.0)
                CALL OPSTOR ('RUNOFCUM', INT(100*RUNOFCUM3)/100.0)
                CALL OPSTOR ('TRWCUM'  , INT(100*TRWCUM3  )/100.0)
                CALL OPSTOR ('EVSWCUM' , INT(100*EVSWCUM3 )/100.0)
                CALL OPSTOR ('PERCCUM', INT(100*PERCCUM3)/100.0)
    !           CALL OPSTOR ('SSOUTCUM', SSOUTCUM3)
    !           CALL OPSTOR ('SSINCUM', SSINCUM3)
              END IF
            END IF
